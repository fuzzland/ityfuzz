...
RUN cargo build --release --locked --features "cmp dataflow evm print_txn_corpus full_trace" --no-default-features
RUN cp target/release/ityfuzz /bins/cli_offchain

# build onchain binary
# RUN sed -i -e 's/"cmp"/"cmp","flashloan_v2"/g' ./Cargo.toml
RUN cargo build --release --locked --features "cmp dataflow evm print_txn_corpus full_trace flashloan_v2" --no-default-features
...
RUN cargo build --release --locked --features "cmp dataflow evm flashloan_v2 print_logs" --no-default-features
