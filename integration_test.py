...
def test_one(path):
    ...
    if b"target bug found" not in p.stderr and b"bug() hit" not in p.stdout and b"[typed_bug]" not in p.stdout and b"[selfdestruct]" not in p.stdout and b"[echidna_bug]" not in p.stdout:
