...
pub fn evm_fuzzer(
    ...
    let coverage_obs_stage = CoverageStage::new(evm_executor_ref.clone(), cov_middleware.clone());
    ...
    if let Some(path) = config.state_comp_oracle {
        let mut file = File::open(path).expect("Failed to open state comp oracle file");
