...
impl ContractLoader {
    fn parse_abi(path: &Path) -> Vec<ABIConfig> {
        ...
        file.read_to_string(&mut data).expect("failed to read abi file");
        ...
        json.iter().filter(
            ...
            abi["inputs"].as_array().expect("failed to parse abi inputs").iter().for_each(|input| {
    ...
    pub fn from_prefix(prefix: &str) -> Self {
        ...
        return Self { contracts: vec![result] }
    ...
    pub fn from_glob(p: &str) -> Self {
            ...
            match i {
                Ok(path) => {
                    ...
                    if path_str.ends_with(".abi") {
                        *prefix_file_count.entry(path_str.replace(".abi", "").clone()).or_insert(0) += 1;
                    } else if path_str.ends_with(".bin") {
                        *prefix_file_count.entry(path_str.replace(".bin", "").clone()).or_insert(0) += 1;
                    } else {
        ...
        ContractLoader {
            contracts: prefix_file_count.iter().filter(|(_, v)| **v == 2).map(|(k, _)| {
                Self::from_prefix((k.to_owned() + &String::from('*')).as_str()).contracts[0].clone()
            }).collect()
        }
    }
}
