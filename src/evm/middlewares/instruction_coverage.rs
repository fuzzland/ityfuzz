...
/// Returns a tuple of (instruction PCs, JUMPDEST PCs)
pub fn instructions_pc(bytecode: &Bytecode) -> (HashSet<usize>, HashSet<usize>) {
    let mut i = 0;
    let bytes = bytecode.bytes();
    let mut complete_bytes = vec![];
    let mut jump_dest = HashSet::new();
    ...
    (complete_bytes.into_iter().collect(), jump_dest)
}
...
pub struct InstructionCoverage {
    ...
    pub jump_dest: HashMap<EVMAddress, HashSet<usize>>,
    pub work_dir: String,
}
...
impl InstructionCoverage {
    ...
    pub fn record_instruction_coverage(&mut self, source_map: &ProjectSourceMapTy) {
        ...
        for (addr, covs) in &self.total_instr_set {
            let mut curr_skip_instructions = self.jump_dest.get(addr).unwrap_or(&HashSet::new()).clone();
        ...
        + Clone,
{
    ...
    unsafe fn on_insert(&mut self, bytecode: &mut Bytecode, address: EVMAddress, host: &mut FuzzHost<VS, I, S>, state: &mut S) {
        ...
        let (pcs, jump_dests) = instructions_pc(&bytecode.clone());
        self.total_instr.insert(address, pcs.len());
        self.total_instr_set.insert(address, pcs);
        self.jump_dest.insert(address, jump_dests);
    }
...
mod tests {
    ...
    fn test_instructions_pc() {
        let (_, pcs) = instructions_pc(&Bytecode::new_raw(
            Bytes::from(
