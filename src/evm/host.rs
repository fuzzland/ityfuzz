    ...
    VS: VMStateT,
{
    ...
    fn call_allow_control_leak(&mut self, input: &mut CallInputs, interp: &mut Interpreter, (out_offset, out_len): (usize, usize), state: &mut S) -> (InstructionResult, Gas, Bytes) {
        ...
        if input.context.scheme == CallScheme::Call {
            ...
            if !self.pc_to_call_hash.contains_key(&(input.context.caller, self._pc, self.jumpi_trace)) {
                self.pc_to_call_hash.insert((input.context.caller, self._pc, self.jumpi_trace), HashSet::new());
            }
            self.pc_to_call_hash
                .get_mut(&(input.context.caller, self._pc, self.jumpi_trace))
                .unwrap()
                .insert(hash.to_vec());
            if self.pc_to_call_hash.get(&(input.context.caller, self._pc, self.jumpi_trace)).unwrap().len() > UNBOUND_CALL_THRESHOLD
                && input_seq.len() >= 4
            {
